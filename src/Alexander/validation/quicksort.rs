...
// QuickSort function from qsort.rs module
use qsort::quickSort;
...
fn main() -> std::io::Result<()> {
    let dir_path = "data_out_quickSort";
        ...
        if path.is_file() {
            ...
            if let (Some(arr_str), Some(low_str), Some(high_str)) = (arr_str, low_str, high_str) {
                ...
                if let (Some(arr_bytes), Some(low_bytes), Some(high_bytes)) = (arr_bytes, low_bytes, high_bytes) {
                    ...
                    if low < 0 || high >= arr.len() as i32 || low > high {
                        continue;
                    }
                    ...
                    // Call your quickSort function from qsort.rs
                    quickSort(Some(&mut arr), low, high);
